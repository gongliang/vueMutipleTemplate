...
const webpackConfig = merge(baseWebpackConfig, {
    ...
    plugins: [
        ...
        new OptimizeCSSPlugin({
            cssProcessorOptions: config.publish.productionSourceMap ? { safe: true, map: { inline: false } } : { safe: true }
        }),
...
let entryMap = getEntries(path.join(__dirname, '../src/entry/'))

Object.keys(entryMap).forEach(key => {
    webpackConfig.plugins.push(
        new HtmlWebpackPlugin({
            filename: `${config.publish.index}/${key}.html`,
            template: `${config.publish.templateRoot}/${key}.html`,
            inject: true,
            minify: {
                removeComments: true,
                collapseWhitespace: true,
